...
function Reports() {
  ...
  const featured = reports.find(r => r.tag === "featured")!;
  const list = reports.filter(r => r.id !== featured.id && (tab === "all" || r.kind === tab));
      ...
      <GlassCard className="p-10 md:p-14 bg-[#151F35]/60 border border-white/8" hover>
        ...
        <div className="relative">
          <div className="font-mono text-[11px] uppercase tracking-[0.28em] text-[#726BFF] font-bold">
            FEATURED · {featured.kind.toUpperCase()} · {featured.period}
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-8">
            <div>
              ...
              <div className="mt-1 font-mono text-2xl text-[#B68CFF] font-bold">{featured.confidence}%</div>
            </div>
            <div>
              <div className="font-mono text-[10px] uppercase text-slate-500 font-semibold">Covers</div>
              <div className="mt-1 flex flex-wrap gap-2">
                {featured.covers?.map(c => <span key={c} className="font-mono text-xs text-slate-350">{c.toUpperCase()}</span>).reduce((prev, curr, i) => i === 0 ? [curr] : [...(prev as any), <span key={`d${i}`} className="text-slate-600">·</span>, curr], [] as any)}
              </div>
            </div>
          ...
          <GlassCard key={r.id} hover className="p-8 bg-[#151F35]/60 border border-white/8">
            ...
            <div className="mt-16 flex items-center justify-between text-xs">
              <span className="flex items-center gap-2">
                ...
                <span className="font-mono uppercase font-bold" style={{ color: r.status === "ready" ? "#31D88A" : "#F4B84F" }}>{r.status}</span>
              </span>
              <span className="font-mono text-[#5CA9FF] font-bold">{r.confidence}% conf</span>
            </div>
          </GlassCard>
        ))}
      </div>
